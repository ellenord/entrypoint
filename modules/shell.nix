  ...
  module = lib.debug.traceSeq "loading system core configuration..." ({
      ...
      programs = {
        zsh = {
          ...
          '';
          shellAliases = {
            rebuild-system = "pushd /home/ellenord/nixos >> /dev/null && sudo nixos-rebuild switch --flake .#desktop --impure && popd >> /dev/null";
            rebuild-home = "pushd /home/ellenord/nixos >> /dev/null && home-manager switch --flake .#ellenord --impure && popd >> /dev/null";
          };
          histSize = 1024;
    ...
    // lib.debug.traceSeq "system shell configuration loaded successfully!" {});
in
  module
